...
class AVDataset(CustomDataset):
    ...
    CLASSES = ("bike", "car", "car_stop", "color_cone", "person")
    ...
    def __init__(self,
        ...
        super(AVDataset, self).__init__(ann_file, pipeline, **kwargs)
            ...
            for ann_file in ann_files:
                ...
                with open(ann_file) as f:
                    ...
                    for si in s:
                        ...
                        poly = np.array(bbox_info[:8], dtype=np.float64).reshape(4, 2)
                        tlx = np.min(poly[:, 0])
                        tly = np.min(poly[:, 1])
                        brx = np.max(poly[:, 0])
                        bry = np.max(poly[:, 1])
